<!DOCTYPE HTML>
<html>

<head>
	<html lang="es">
	<meta name="viewport" content="initial-scale=1, maximum-scale=1" />
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width" />
	<title>2MG - NANO4LIFE.</title>
	<meta name="author" content="Brandemma" />
	<meta name="description" content="2MG es una empresa peruana que se dedica a la importación y exportación de productos, servicios y soluciones en Procurement de Ingeniería."/>

	<!--================== 	ENLACE HOJA DE ESTILOS (INICIA)  =====================-->

	<link rel="stylesheet" type="text/css" href="css/estilos.css">

	<!--================== 	ENLACE HOJA DE ESTILOS (TERMINA)  =====================-->

	<!--================== 	ENLACE ICONO (INICIA)  =====================-->

	<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css">

	<!--================== 	ENLACE ICONO (TERMINA)  =====================-->

	<!--================== 	ENLACE GOOGLE ANALYTICS (INICIA)  =====================-->

	<script>
	  (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
	  (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
	  m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
	  })(window,document,'script','https://www.google-analytics.com/analytics.js','ga');

	  ga('create', 'UA-79840828-1', 'auto');
	  ga('send', 'pageview');

	</script>

	<!--================== 	ENLACE GOOGLE ANALYTICS (TERMINA)  =====================-->

</head>

<body>
	<header>
		<?php
			include 'menu.php';
		?>
	</header>
	
	<div class="contenedor-xado">
		
		<img class="banner-xado" src="img/banner-xado.jpg">

		<div class="descripcion-xado">
			<div class="texto-xado-izq">
				<img src="img/marco-nano4life.jpg">
			</div>
			
			<div class="texto-xado-der">
				<p>A NANO4LIFE® es la marca lider mundial en revestimientos de Nanotecnología que fueron específicamente desarrollados para sustituir a los convencionales para diferentes aplicaciones.<br><br>
				Los revestimientos NANO4LIFE® tienen como base SiO2 (Cuarzo puro de dióxido de silicio) nuestros productos están hechos a partir del proceso de fabricación sol-gel de forma que obtenemos y garantizamos revestimientos altamente duraderos, seguros y reconocidos por su rapidez y facilidad de aplicación además son totalmente ecológicos.<br><br>
				Las superficies tratadas con soluciones NANO4LIFE® se vuelven “cómo por arte de magia” facilmente limpiables, hidrofóbicas, impiden la adherencia de partículas, además cómo son muy resistentes a los rayos UV permitan aumentar la vida útil y conservar las características originales de los materiales sin alterar sus propiedades , apariencias o su capacidad de transpiración aunque estos sean porosos.</p>
			</div>
			<div class="clear"></div>

			<h1 class="titulo-xado2">Productos NANO4LIFE</h1>

			<div class="subtitulo-xado">
				<h2>Productos &nbsp; <i class="fa fa-sort-desc" aria-hidden="true"></i></h2>
				<p>Descubra lo que pueden hacer los productos Nano4Life® en su día a día.	</p>
			</div>

			<div class="contenedor-productos-xado">

				<div class="producto-xado">
					<img src="img/producto-nano4bathcare.jpg">
					<h3>NANO4BATHCARE</h3>
					<p class="descripcion-producto-xado">Protege las superfícies a través de una barrera 100% invisíble que impide la adherencia...</p>
					<p class="codigo-producto-xado">CODIGO: 97590</p>

					<a class="mas-info-producto-xado" href="nano4bathcare.php"><i class="fa fa-plus-circle" aria-hidden="true"></i>&nbsp;&nbsp;MÁS INFO</a>
				</div>

				<div class="producto-xado">
					<img src="img/producto-nano4carglass.jpg">
					<h3>NANO4CARGLASS</h3>
					<p class="descripcion-producto-xado">Protección 100% invisíble, que impide la adherencia de líquidos y suciedad al pára-brisas...</p>
					<p class="codigo-producto-xado">CODIGO: 97590</p>

					<a class="mas-info-producto-xado" href="nano4carglass.php"><i class="fa fa-plus-circle" aria-hidden="true"></i>&nbsp;&nbsp;MÁS INFO</a>
				</div>

				<div class="producto-xado">
					<img src="img/producto-nano4carprotect.jpg">
					<h3>NANO4CARPROTECT</h3>
					<p class="descripcion-producto-xado">Protección 100% invisíble para las superfícies pintadas de los vehículos. Crea una barrera...</p>
					<p class="codigo-producto-xado">CODIGO: 97590</p>

					<a class="mas-info-producto-xado" href="nano4carprotect.php"><i class="fa fa-plus-circle" aria-hidden="true"></i>&nbsp;&nbsp;MÁS INFO</a>
				</div>

				<div class="producto-xado">
					<img src="img/producto-nano4chromemetal.jpg">
					<h3>NANO4CHROMEMETAL</h3>
					<p class="descripcion-producto-xado">Forma una película protectora 100% invisíblel, que impide la aparición de manchas...</p>
					<p class="codigo-producto-xado">CODIGO: 97590</p>

					<a class="mas-info-producto-xado" href="nano4chromemetal.php"><i class="fa fa-plus-circle" aria-hidden="true"></i>&nbsp;&nbsp;MÁS INFO</a>
				</div>

				<div class="producto-xado">
					<img src="img/producto-nano4carprotect.jpg">
					<h3>NANO4CSCCAR</h3>
					<p class="descripcion-producto-xado">Producto 3 en 1 que efectua limpieza, pulimento y protección. LIMPIEZA: Por acción...</p>
					<p class="codigo-producto-xado">CODIGO: 97590</p>

					<a class="mas-info-producto-xado" href="nano4csccar.php"><i class="fa fa-plus-circle" aria-hidden="true"></i>&nbsp;&nbsp;MÁS INFO</a>
				</div>

				<div class="producto-xado">
					<img src="img/producto-nano4csc4yatch.jpg">
					<h3>NANO4CSCYATCH</h3>
					<p class="descripcion-producto-xado">LIMPEZA: Gracias a la acción de dos poderosos componentes nanotecnológicos...</p>
					<p class="codigo-producto-xado">CODIGO: 97590</p>

					<a class="mas-info-producto-xado" href="nano4cscyatch.php"><i class="fa fa-plus-circle" aria-hidden="true"></i>&nbsp;&nbsp;MÁS INFO</a>
				</div>

				<div class="producto-xado">
					<img src="img/producto-nano4glassceramic.jpg">
					<h3>NANO4GLASSCERAMIC</h3>
					<p class="descripcion-producto-xado">Protección100% invisiblel, que impide la adherencia de líquidos  suciedad en las superfícies...</p>
					<p class="codigo-producto-xado">CODIGO: 97590</p>

					<a class="mas-info-producto-xado" href="nano4glassceramic.php"><i class="fa fa-plus-circle" aria-hidden="true"></i>&nbsp;&nbsp;MÁS INFO</a>
				</div>

				<div class="producto-xado">
					<img src="img/producto-nano4plastic.jpg">
					<h3>NANO4PLASTIC</h3>
					<p class="descripcion-producto-xado">Protección 100% invisíble, que impide la adherencia de líquidos y suciedad en las superfícies...</p>
					<p class="codigo-producto-xado">CODIGO: 97590</p>

					<a class="mas-info-producto-xado" href="nano4plastic.php"><i class="fa fa-plus-circle" aria-hidden="true"></i>&nbsp;&nbsp;MÁS INFO</a>
				</div>


			</div>

		</div>

		<?php
			include 'contacto.php';
		?>
	</div>

	<?php
		include 'pie-pagina.php';
	?>
</body>

</html>